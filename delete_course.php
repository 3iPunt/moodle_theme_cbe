...
/**
 * Delete course if user is teacher.
 *
 * @package     theme_cbe
...
if ($SITE->id == $course->id || !\theme_cbe\course_user::is_teacher($id)) {
...
if ($delete === md5($course->timemodified)) {
    ...
    echo $OUTPUT->continue_button($dashboard);
    echo $OUTPUT->footer();
...
if (!async_helper::is_async_pending($id, 'course', 'backup')) {
    ...
    $continueurl = new moodle_url('/theme/cbe/delete_course.php', array('id' => $course->id, 'delete' => md5($course->timemodified)));
    $continuebutton = new single_button($continueurl, get_string('delete'), 'post');
    echo $OUTPUT->confirm($message, $continuebutton, $dashboard);
} else {
